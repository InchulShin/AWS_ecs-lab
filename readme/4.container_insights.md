...
애플리케이션 설정 이 섹션에서는 컨테이너 인사이트를 설정합니다. 시작하려면 ECS에 환경과 프런트엔드 서비스를 배포해야 합니다. 이전 장에서 마이크로서비스를 배포했다면 이 단계를 건너뛰고 다음 페이지로 넘어갈 수 있습니다.
...
ECS 클러스터에서 Container Insights 유효성 검사가 활성화되었습니다. 다음 명령을 실행
